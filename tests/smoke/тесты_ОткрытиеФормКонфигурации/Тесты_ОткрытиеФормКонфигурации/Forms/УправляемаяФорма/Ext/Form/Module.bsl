...
Процедура ЗаполнитьНаборТестов(НаборТестов, КонтекстЯдраПараметр) Экспорт
	...
	ОчереднойВидМетаданного = "РегистрыСведений";
	Если НЕ ВидМетаданныхИсключенИзТестирования(ОчереднойВидМетаданного) Тогда

		ОписанияТестов = Новый Массив;

		ДобавлятьОбработки = Ложь;
		ДобавитьТестыДляРегистровСведенийСервер(
			КонтекстЯдра.Объект,
			ОписанияТестов, ТолькоУправляемыеФормы,
...
&НаСервере
Функция ПолучитьСуществующуюЗаписьРегистраИВернутьКлючЗаписиСервер(Мета_ПолноеИмя)
	ИД = ПолучитьВидМетаданного(Мета_ПолноеИмя);

	ОсновнойОбъект = Объект();
	КлючЗаписи = ОсновнойОбъект.ПолучитьСуществующуюЗаписьРегистраИВернутьКлючЗаписи(ИД, Ложь,
		"ТестДолжен_ОткрытьФормуСуществующейЗаписиРегистраСведений");

	Возврат КлючЗаписи;
КонецФункции

#Область Тесты_РегистрыСведений

&НаКлиенте
Процедура ТестДолжен_ОткрытьФормуНовойЗаписиРегистраСведений(Мета_ПолноеИмя) Экспорт     
	
	ТестироватьФормуЗаписиРегистраСведений(Мета_ПолноеИмя, Неопределено);

КонецПроцедуры

&НаКлиенте
Процедура ТестДолжен_ОткрытьФормуСуществующейЗаписиРегистраСведений(Мета_ПолноеИмя) Экспорт

	КлючСуществующейЗаписи = ПолучитьСуществующуюЗаписьРегистраИВернутьКлючЗаписиСервер(Мета_ПолноеИмя);
	Если ЗначениеЗаполнено(КлючСуществующейЗаписи) Тогда
		ТестироватьФормуЗаписиРегистраСведений(Мета_ПолноеИмя, КлючСуществующейЗаписи);
	КонецЕсли;
...
Функция ПодробноеПредставлениеПрефикса(ПрефиксТеста)
	...
	ИначеЕсли ПрефиксТеста = "НовыеЗаписи" Тогда

		Результат = "открытие формы записи регистра при создании";

	ИначеЕсли ПрефиксТеста = "СуществующиеЗаписи" Тогда

		Результат = "открытие формы существующей записи регистра";
...
&НаСервере
Процедура ДобавитьТестыДляРегистровСведенийСервер(Знач ОбъектКонтекстаЯдра, ОписанияТестов, ТолькоУправляемыеФормы,
		ИменаОсновныхФорм_Объекты_Сервер, ИменаОсновныхФорм_Списки_Сервер, НастройкаГруппировки)

	ОсновнойОбъект = Объект();
	КонтекстЯдра = КонтекстЯдраНаСервере(ОбъектКонтекстаЯдра);

	СписокИсключений_Существующие = ОсновнойОбъект.ПолучитьСписокИсключений_РегистрыСведений_Существующие();
	СписокИсключений_Новые = ОсновнойОбъект.ПолучитьСписокИсключений_РегистрыСведений_Новые();
	СписокИсключений_Списки = ОсновнойОбъект.ПолучитьСписокИсключений_РегистрыСведений_Списки();

	Счетчик = 0;

	ОтобранныеМетаданные = ОтобратьМетаданные("РегистрыСведений", Метаданные.РегистрыСведений, Объект.Настройки,
		ОсновнойОбъект, Неопределено, КонтекстЯдра);
	...
	Для Каждого ОписаниеМетаданного Из ОтобранныеМетаданные Цикл 
		...
		НаборОписанийТестов = ДобавитьГруппировкуЕслиНужно("РегистрыСведений", "РегистрСведений." + МетаОбъект.Имя,
				ОписанияТестов, НастройкаГруппировки, Счетчик);

		Если МетаОбъект.РежимЗаписи = Метаданные.СвойстваОбъектов.РежимЗаписиРегистра.Независимый Тогда

			Если ПравоДоступа("Изменение", МетаОбъект) Тогда

				НастройкиТеста = НастройкиТеста(ТолькоУправляемыеФормы,
						СписокИсключений_Новые, ИменаОсновныхФорм_Объекты_Сервер,
						"ТестДолжен_ОткрытьФормуНовойЗаписиРегистраСведений", "НовыеЗаписи");
			...
			НастройкиТеста = НастройкиТеста(ТолькоУправляемыеФормы,
					СписокИсключений_Существующие, ИменаОсновныхФорм_Объекты_Сервер,
					"ТестДолжен_ОткрытьФормуСуществующейЗаписиРегистраСведений", "СуществующиеЗаписи");
...
&НаКлиенте
Процедура ТестироватьФормуЗаписиРегистраСведений(Мета_ПолноеИмя, КлючЗаписи) 
