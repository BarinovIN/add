...
Функция ПолучитьСписокИсключений_Справочники_Списки() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Справочники, ВидыПроверок.Списки);
...
Функция ПолучитьСписокИсключений_Справочники_Существующие() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Справочники, ВидыПроверок.Существующие);
...
Функция ПолучитьСписокИсключений_Справочники_Новые() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Справочники, ВидыПроверок.Новые);
...
Функция ПолучитьСписокИсключений_Документы_Списки() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Документы, ВидыПроверок.Списки);
...
Функция ПолучитьСписокИсключений_Документы_Существующие() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Документы, ВидыПроверок.Существующие);
...
Функция ПолучитьСписокИсключений_Документы_ПеренестиДату() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Документы, ВидыПроверок.ПеренестиДату);
...
Функция ПолучитьСписокИсключений_Документы_Новые() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Документы, ВидыПроверок.Новые);
...
Функция ПолучитьСписокИсключений_Отчеты() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Отчеты, "");
...
Функция ПолучитьСписокИсключений_Обработки() Экспорт
	...
	Возврат ПолучитьИсключения(ВидыМетаданных.Обработки, "");
...
Процедура ДобавитьТестыФормСправочников(НаборТестов, НастройкаГруппировки)
	...
	Для Каждого МетаОбъект Из Метаданные.Справочники Цикл
		...
		Если НастройкаГруппировки.ГруппироватьПоВидуОбъекта Тогда
			НаборТестов.НачатьГруппу("Справочник." + ИмяМетаданного, СтрогийПорядокВыполнения);
		КонецЕсли;
...
Процедура ДобавитьТестыФормДокументов(НаборТестов, НастройкаГруппировки)
	...
	Для Каждого МетаОбъект Из Метаданные.Документы Цикл
		...
		Если НастройкаГруппировки.ГруппироватьПоВидуОбъекта Тогда
			НаборТестов.НачатьГруппу("Документ." + ИмяМетаданного, СтрогийПорядокВыполнения);
		КонецЕсли;
...
Процедура ДобавитьТестПоМенеджеруМетаданных(НаборТестов, менеджерМетаданного, имяТеста, СписокИсключений, ИменаОсновныхФорм)
	...
	КлючМетаданных = "Обработки";
	КлючМетаданного = "Обработка";
	Если ЭтоОтчеты Тогда
		КлючМетаданных = "Отчеты";
		КлючМетаданного = "Отчет";
	КонецЕсли;
	...
	Для Каждого МетаОбъект Из менеджерМетаданного Цикл
		...
		Если НастройкаГруппировки.ГруппироватьПоКоличеству Тогда
			Если Счетчик % НастройкаГруппировки.КоличествоВГруппе = 0 Тогда
				ИмяГруппы = КлючМетаданных + " (" + Строка(Счетчик + 1) + "..." +
		...
		Если НастройкаГруппировки.ГруппироватьПоВидуОбъекта Тогда
			НаборТестов.НачатьГруппу(КлючМетаданного + "." + ИмяМетаданного, СтрогийПорядокВыполнения);
		КонецЕсли;
...
Процедура ДобавитьТестДляМетаданного_ОдинОбъект(НаборТестов, Знач МетаОбъект, Знач имяТеста,
	...
	Если СписокИсключений = Ложь Или СписокИсключений.НайтиПоЗначению(ИмяМетаданного) <> Неопределено Тогда
	...
	Для каждого МетаФорма Из МетаФормы Цикл
		...
		Если Не ПропускатьТест Тогда
			...
			ОтносительныйПутьФормы = ИмяМетаданного + ".Форма." + МетаФорма.Имя;
...
Процедура ДобавитьТестДляОдногоМетаданного_Объекты(НаборТестов, Знач МетаОбъект, Знач имяТеста,
	...
	Для каждого МетаФорма Из МетаФормы Цикл
		...
		Если ПропускатьТест Или ПропускатьТестФормы Тогда
			...
			ИсключатьТестИзСписка = КонтекстЯдра.ИсключатьПропускаемыеТестыИзСписка;
		Иначе
			ПараметрыТеста = НаборТестов.ПараметрыТеста(ИмяМетаданного, МетаФорма_ПолноеИмя, МетаФорма, ЕстьТранзакция);
			ВремИмяТеста = имяТеста;
		КонецЕсли;
...
Процедура ЗакрытьФорму()
	...
	ТипФормы = ТипЗнч(ТестируемаяФорма);

	Если ТипФормы <> Тип("Форма") И ТипФормы <> Тип("УправляемаяФорма") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьНастройки()
	...
	Если Не ЗначениеЗаполнено(Настройки) Тогда
		Настройки = ПлагинНастроек.ПолучитьНастройку(КлючНастройкиУстаревший());   // обратная совместимость
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Настройки) Тогда
...
Функция СоздатьНаборНастроекПоУмолчанию() Экспорт
	...
	Рез.Вставить("ПроверятьТолькоИзбранные", Новый Массив);
...
// Функция - Возвращается тексты исключений в соответствии с настройками
// 
// Возвращаемое значение:
//  Массив - Массив строк
//
Функция ТекстыИсключений() Экспорт 
...
Функция ПолучитьСуществующийДокументОбъект(ИД, НужноИсключениеЕслиНеНайденоДокументов, ДопОписаниеДляИсключения)
	...
	Если Не Успешно Тогда
		Если НужноИсключениеЕслиНеНайденоДокументов Тогда
			ВызватьИсключение
			СтрШаблон("%1: выборка.Следующий() - не найдено существующих документов %2! ",
...
Функция ПолучитьОбъектСуществующегоЭлементаСправочника(ИД, НужноИсключениеЕслиНеНайденоСправочников,
	...
	Если Не Успешно Тогда
		Если НужноИсключениеЕслиНеНайденоСправочников Тогда
			ВызватьИсключение
			СтрШаблон("%1: выборка.Следующий() - не найдено существующих элементов справочника %2! ",
				ДопОписаниеДляИсключения, Ид);
		Иначе
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьРезультатЗапросаСуществующийЭлементСправочника(ИД, Отбор)
	...
	Если Отбор.Количество() <> 0 Тогда
		...
		Для Каждого ЭлементОтбора Из Отбор Цикл
			Условия.Добавить("ОбъектЗапроса." + ЭлементОтбора.Ключ + " = " + ЭлементОтбора.Значение);
		КонецЦикла;
		ТекстЗапроса = ТекстЗапроса + СтрСоединить(Условия, " И ") + " И Не ОбъектЗапроса.ПометкаУдаления";
	Иначе
		ТекстЗапроса = ТекстЗапроса + " ГДЕ Не ОбъектЗапроса.ПометкаУдаления";
	КонецЕсли;
...
Функция СоздатьЭлемент(ИД, Наименование, ПроверяемоеПравоДоступа = Неопределено)
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Не удалось найти объект метаданных по имени ""%1"".'"), ИД);
	КонецЕсли;
	Мета = Элем.Метаданные();

	Если Не РазрешеноИзменятьОбъектВМоделиСервиса(Мета) Тогда
		ЗаписьЖурналаРегистрации(Метаданные().Синоним, УровеньЖурналаРегистрации.Информация, Мета, , СтрШаблон("Метаданные ""%1"" пропущены", Мета.ПолноеИмя()));
		Возврат Неопределено;
	КонецЕсли;
...
Функция РазрешеноИзменятьОбъектВМоделиСервиса(МетаОбъект)
	МОбщегоНазначения = Неопределено;
	Попытка
		МОбщегоНазначения = Вычислить("ОбщегоНазначения");
	Исключение
		Возврат Истина;   // Если нет модуля ОбщегоНазначения, врядли там включено разделение по областям
	КонецПопытки;
...
Функция ПолучитьОбъектСуществующегоБизнесПроцесса(ИД, НужноИсключениеЕслиНеНайденоБизнесПроцессов,
	...
	Если Не Успешно Тогда
		Если НужноИсключениеЕслиНеНайденоБизнесПроцессов Тогда
			ВызватьИсключение
			СтрШаблон("%1: выборка.Следующий() - не найдено существующих бизнес процессов %2! ",
				ДопОписаниеДляИсключения, Ид);
		Иначе
			Возврат Неопределено;
		КонецЕсли;
...
Функция ПолучитьСуществующуюЗаписьРегистраИВернутьКлючЗаписи(ИД, НужноИсключениеЕслиНеНайденоЗаписей, ДопОписаниеДляИсключения) Экспорт
	...
	Если Не Успешно Тогда
		Если НужноИсключениеЕслиНеНайденоЗаписей Тогда
			ВызватьИсключение
			СтрШаблон("%1: выборка.Следующий() - не найдено существующих записей %2! ",
				ДопОписаниеДляИсключения, Ид);
		Иначе
			Возврат Неопределено;
		КонецЕсли;
...
Процедура ВывестиСообщение(ТекстСообщения, Знач УровеньЖР = Неопределено, Сообщать = Ложь)
	...
	Если ВыводитьСообщенияВЖурналРегистрации Тогда
		...
		КонецЕсли;
		ЗаписьЖурналаРегистрации(ИмяСобытия(), УровеньЖР, , , ТекстСообщения);
	КонецЕсли;
...
Функция ИмяСобытия()
	Возврат "VanessaADD.Дымовые.Тесты_ОткрытиеФормКонфигурации"; // Что бы было идентично с записями из формы
КонецФункции
