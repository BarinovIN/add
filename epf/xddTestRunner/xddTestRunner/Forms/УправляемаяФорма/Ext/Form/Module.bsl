...
Процедура ПервичнаяНастройка()
	...
	НастройкиТестов = ПлагинНастроек.ПолучитьНастройки();
	Для Каждого КлючЗначение Из НастройкиТестов Цикл
	...
	Если ЭтоПакетныйЗапуск Тогда
		...
		Объект.ИмяФайлаЛогВыполненияСценариев = ЗначениеНастройки("ИмяФайлаЛогВыполненияСценариев", "", ПлагинНастроек);
...
Функция ПодробноеПредставлениеОшибки_(ИнформацияОбОшибке)
	...
	Если Объект.ВыводитьПодробноеПредставлениеОшибки Тогда
		...
		ТекстОшибки = ПолучитьСообщениеБезСтекаВызововОтБраузераТестов(ТекстОшибки);
	Иначе
