...
Перем УжеИнициализировали; // проверка первичной настройки
...
Процедура Инициализация()
	Если УжеИнициализировали = Истина Тогда
		Возврат;
	КонецЕсли;
	УжеИнициализировали = Истина;
...
// Получить объект плагина, внешней обработки или отчета, встроенной обработки или отчета
//
// Параметры:
//  Идентификатор	 - Строка - имя\идентификатор плагина\внешнего файла
// 
// Возвращаемое значение:
//   Произвольный - сам объект 
//
Функция Плагин(Знач Идентификатор) Экспорт

	Имя = ПолучитьИдентификаторБезПрефиксаПодсистемы(Идентификатор);

	Результат = ПлагинИзКешаПлагинов(Имя);
	Если Результат <> Неопределено Тогда
		Возврат Результат;
	КонецЕсли;

	ЕстьПлагины = Плагины <> Неопределено; // в случае неиспользования на сервере через ВнешниеОбработки.Создать("xddTestRunner")
...
#Если Сервер Тогда
	Возврат ПлагинИзСерверногоКеша(Имя);
#Иначе
	Возврат Имя;
#КонецЕсли
...
Процедура ИнициализацияНаСервере(Знач ОбъектКонтекстаЯдра) Экспорт
	...
	КопируемыеСвойства = "Плагины, ПутьФайлаНастроек, АдресВременногоХранилища";
	ЗаполнитьЗначенияСвойств(КонтекстЯдра, ОбъектКонтекстаЯдра, КопируемыеСвойства);
...
Функция ПлагинИзКешаПлагинов(Знач Имя)
	Если ЗначениеЗаполнено(КешПлагины) Тогда
		Результат = Неопределено;
		Если КешПлагины.Свойство(Имя, Результат) Тогда
			Возврат Результат;
		КонецЕсли;
	КонецЕсли;
...
Функция ПлагинИзСерверногоКеша(Знач Имя)
	...
	Если Не ЗначениеЗаполнено(НастройкиПродукта) Тогда
		НастройкиПродукта = НовыеНастройкиПродукта();
	КонецЕсли;
	КешПлагины = НастройкиПродукта.КешПлагины;

	Результат = Неопределено;
	Если Не КешПлагины.Свойство(Имя, Результат) Или Результат = Неопределено Тогда
		Результат = СоздатьОбъектПлагина(Имя);

		КешПлагины.Вставить(Имя, Результат);
	...
	Для каждого ДочернийУзел Из СтрокиКонтейнера Цикл
		...
		ИначеЕсли ДочернийУзел.Тип = ЭтотОбъект.ТипыУзловДереваТестов.Элемент Тогда
			...
			Если ДочернийУзел.ПропуститьТест Тогда
				...
				ДочернийРезультатТестирования.Состояние = СостоянияТестов.НеРеализован;
			...
			Если МожноВыполнять Тогда
				...
				Иначе
					Если Контекст = Неопределено Или Контекст.ИспользуемоеИмяФайла <> ДочернийУзел.Путь Тогда 
...
Функция ВыполнитьИнициализациюКонтекста(КонтекстВыполнения, РезультатТестирования)
	...
	Исключение
		...
		Если (ТипЗнч(КонтекстВыполнения) = Тип("ОбщийМодуль") Или ЭтоМодульМенеджера(КонтекстВыполнения))
...
Процедура ДополнитьСтрокуПараметровКонтекстаОбщегоМодуля(КонтекстВыполнения, СтрокаПараметров)
	...
	Если ТипЗнч(КонтекстВыполнения) = Тип("ОбщийМодуль")
		Или ЭтоМодульМенеджера(КонтекстВыполнения) Тогда
...
Функция ЭтоМодульМенеджера(КонтекстВыполнения)
	КонтекстВыполнеияСтрокой = Строка(КонтекстВыполнения);
	
	Возврат СтрНайти(КонтекстВыполнеияСтрокой, "Менеджер") > 0 Или СтрНайти(КонтекстВыполнеияСтрокой, "Manager") > 0;
КонецФункции
...
// Новые настройки продукта
// 
// Возвращаемое значение:
//   Структура - настройки:
//		* КешПлагины - Структура
//
Функция НовыеНастройкиПродукта() Экспорт

	НастройкиПродукта = Новый Структура;
	НастройкиПродукта.Вставить("КешПлагины", Новый Структура);

	Возврат НастройкиПродукта;
	...
	Если ТипЗнч(СчитанныеНастройки) = Тип("Структура") Тогда
		Для каждого КлючЗначение Из СчитанныеНастройки Цикл
			Если ЭтотОбъект.Настройки.Свойство(КлючЗначение.Ключ) И ТипЗнч(КлючЗначение.Значение) = ТипЗнч(ЭтотОбъект.Настройки[КлючЗначение.Ключ]) Тогда
				ЭтотОбъект.Настройки[КлючЗначение.Ключ] = КлючЗначение.Значение;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
