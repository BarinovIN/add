...
Процедура ПрочитатьДанныеИзМакета(ПротоДанные, ТабличныйДокумент, ИмяКолонкиЗамещения)
...
Процедура ПрочитатьДанныеИзТекстовогоДокумента(ПротоДанные, ТабличныйДокумент)
...
Процедура ПроверитьКолонки(КолонкиМакета,ИмяКолонкиЗамещения)
...
Функция СоздатьОбъект_Справочник(ПротоДанные,ПротоОбъект)
	...
	Если Объект = Неопределено Тогда
		...
		Если ПротоОбъект.Режимы.Предопределенный Тогда
			...
			Если НеСоздаватьПредопределенные Тогда
				Попытка
					Ссылка = СпрМенеджер[ПротоОбъект.ИмяПредопределенного];
					Возврат Ссылка;
				Исключение
					Ошибка = ОписаниеОшибки();
	...
	Для каждого мдТабЧасть из Объект.Метаданные().ТабличныеЧасти Цикл
		...
		ЕстьИспользование = ЕстьРеквизитУОбъекта(мдТабЧасть, "Использование");
		Если (ЭтоРежимГруппа И ЕстьИспользование И мдТабЧасть.Использование<>Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента)
			Или (Не ЭтоРежимГруппа И ЕстьИспользование И мдТабЧасть.Использование<>Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы) Тогда
...
Функция ОчиститьОбъект(Знач Объект, Знач НовыйПустойОбъект, ПропускаемыеРеквизиты)
	...
	Иначе
		...
		Исключение
			...
			ИначеЕсли Найти(ОписаниеОшибки, "ValueType") > 0 Тогда
				ЗаполнитьЗначенияСвойств(Рез, НовыйПустойОбъект, , СтрокаРеквизитов + ",ValueType");
			Иначе
				ВызватьИсключение;
			КонецЕсли;
		КонецПопытки;
...
Функция ПолучитьПропускаемыеРеквизиты(Знач Мд, Знач ИсключатьДляГруппы = Ложь)
	...
	Если КорневойТипКонфигурации = ВернутьЗначениеСоответствия("Справочник") Тогда
		...
		Для каждого мдРеквизит из Мд.Реквизиты Цикл
			ЕстьИспользование = ЕстьРеквизитУОбъекта(мдРеквизит, "Использование");
			Если ИсключатьДляГруппы И ЕстьИспользование И мдРеквизит.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента Тогда
				РезМассив.Добавить(мдРеквизит.Имя);
			ИначеЕсли Не ИсключатьДляГруппы И ЕстьИспользование И мдРеквизит.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы Тогда
				РезМассив.Добавить(мдРеквизит.Имя);
			КонецЕсли;
		КонецЦикла;
		Для каждого мдТабЧасть из Мд.ТабличныеЧасти Цикл
			ЕстьИспользование = ЕстьРеквизитУОбъекта(мдТабЧасть, "Использование");
			Если ИсключатьДляГруппы И ЕстьИспользование И мдТабЧасть.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляЭлемента Тогда
				РезМассив.Добавить(мдТабЧасть.Имя);
			ИначеЕсли Не ИсключатьДляГруппы И ЕстьИспользование И мдТабЧасть.Использование = Метаданные.СвойстваОбъектов.ИспользованиеРеквизита.ДляГруппы Тогда
				РезМассив.Добавить(мдТабЧасть.Имя);
			КонецЕсли;
		КонецЦикла;
...
Функция ПолучитьЗначениеПоля(ПротоДанные,стрПоле,НаборМдРеквизитов)
	...
	Если ПустаяСтрока(стрПоле.Режим) Тогда
		...
		Иначе
			Если ЗначениеЗаполнено(НаборМдРеквизитов)
					И НаборМдРеквизитов[стрПоле.Имя].Тип = Новый ОписаниеТипов("ХранилищеЗначения") И
					стрПоле.Значение <> "ХранилищеЗначения" Тогда
				Значение = XMLЗначение(Тип("ХранилищеЗначения"), стрПоле.Значение);
            ИначеЕсли ЗначениеЗаполнено(НаборМдРеквизитов)
                	И НаборМдРеквизитов[стрПоле.Имя].Тип = Новый ОписаниеТипов("ОписаниеТипов") Тогда
                ХранЗнч = XMLЗначение(Тип("ХранилищеЗначения"), стрПоле.Значение);
                Значение = ХранЗнч.Получить();
            Иначе
	...
	Попытка
		...
		Иначе
			...
			Если Не ЕстьМетаданныеРеквизита И Режим = "ПРЕДОПРЕДЕЛЕННЫЙ" Тогда
				Если БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип( ТипЗначения.Типы()[0] ) Тогда
					Значение = ПолучитьЗначениеТочкиБизнесПроцесса(стрПоле.Значение, стрПоле.ДополнительныйТип);
					УжеВычислилиЗначение = Истина;
				Иначе
					Значение = Неопределено;
		...
		Если Не УжеВычислилиЗначение и ЕстьМетаданныеРеквизита Тогда
			...
			ИначеЕсли Режим = "ГУИД" Тогда
				// При переборе возможных типов субконто, если в типы входит перечисление, то тест падает при попытке получения значения по ссылке.
				Если Найти(ПолноеИмяТипаРеквизита, ВернутьЗначениеСоответствия("Перечисление")) = 1 Тогда
					Значение = "Объект не найден";
				ИНаче
...
Функция ПолучитьЗначениеТочкиБизнесПроцесса(Значение_Строкой, ТипЗначения_Строкой = "")
	Значение = Неопределено;
	Для каждого БизнесПроцессМенеджер из БизнесПроцессы Цикл
		ТочкиМаршрута = БизнесПроцессМенеджер.ТочкиМаршрута;

		Если ЗначениеЗаполнено(ТипЗначения_Строкой)
			И ТочкиМаршрута.Количество() > 0
			И ТипЗнч(ТочкиМаршрута[0]) <> Тип(ТипЗначения_Строкой) Тогда
...
Функция ТипЗначенияТочкиБизнесПроцессаСтрокой(ТочкаМаршрута)
	...
	Для каждого БизнесПроцессМенеджер из БизнесПроцессы Цикл
		...
		Если ТочкиМаршрута.Количество() > 0
			И ТипЗнч(ТочкиМаршрута[0]) = ТипЗнч(ТочкаМаршрута) Тогда

			Значение = СтрЗаменить(Строка(БизнесПроцессМенеджер), "БизнесПроцессМенеджер.", "ТочкаМаршрутаБизнесПроцессаСсылка.");
			Возврат Значение
...
Функция НаборРеквизитовМетаданных(КорневойТип, МетаданныеОбъекта)
	...
	Если КорневойТип = ВернутьЗначениеСоответствия("Справочник")
		...
		Если ЗаписыватьОбщиеРеквизиты Тогда
			ДобавитьРеквизитыМетаданныхВНаборРеквизитов(Метаданные.ОбщиеРеквизиты, НаборМдРеквизитов);
		КонецЕсли;
...
Процедура ДобавитьРеквизитыМетаданныхВНаборРеквизитов(МдРеквизиты, НаборМдРеквизитов, КорневойТип = "",СтандартныеРеквизиты = Ложь)
	Для Каждого МдРеквизит Из МдРеквизиты Цикл
		Если СтандартныеРеквизиты И КорневойТип = "РегистрБухгалтерии" и Сред(МдРеквизит.Имя,1,8) = "Субконто" Тогда

			НаборМдРеквизитов.Вставить("Субконто" + "Дт" + Прав(МдРеквизит.Имя,1), МдРеквизит);
			НаборМдРеквизитов.Вставить("Субконто" + "Кт" + Прав(МдРеквизит.Имя,1), МдРеквизит);

		ИначеЕсли СтандартныеРеквизиты И КорневойТип = "РегистрБухгалтерии" Тогда
			НаборМдРеквизитов.Вставить(МдРеквизит.Имя, МдРеквизит);

		ИначеЕсли КорневойТип = "РегистрБухгалтерии" И Не МдРеквизит.Балансовый Тогда
...
Функция ВыгружаемыеСтандартныеРеквизиты(ОписательОбъекта, КорневойТипКонфигурации)
	...
	ИначеЕсли КорневойТипКонфигурации = ВернутьЗначениеСоответствия("ПланВидовХарактеристик") Тогда
		...
		ВыгружаемыеСтандартныеРеквизиты.Добавить(ВернутьЗначениеСоответствия("Наименование"));
		ВыгружаемыеСтандартныеРеквизиты.Добавить(ВернутьЗначениеСоответствия("ПометкаУдаления"));
        ВыгружаемыеСтандартныеРеквизиты.Добавить(ВернутьЗначениеСоответствия("ТипЗначения"));
        ВыгружатьКод = Ложь;
		Если ОписательОбъекта.Свойство("ВыгружатьКод", ВыгружатьКод) и ВыгружатьКод Тогда
			ВыгружаемыеСтандартныеРеквизиты.Добавить(ВернутьЗначениеСоответствия("Код"));
		КонецЕсли;
...
Процедура ПолучитьПредставлениеИРежимПоискаПоЗначениюРеквизита(Объект, ЗначениеРеквизита, ТипРеквизита, МассивДанных, резПредставлениеЗначенияРеквизита, резРежимПоиска, резДополнительныйТипЗначения)
    ...
    ИначеЕсли ТипРеквизита = Тип("ОписаниеТипов") Тогда
        ХранЗнч = Новый ХранилищеЗначения(ЗначениеРеквизита);
        ПредставлениеЗначенияРеквизита = XMLСтрока(ХранЗнч);
	ИначеЕсли ЛиСсылкаНаОбъектБД_ирЛкс(ЗначениеРеквизита) Тогда
		...
		Иначе
			...
			Если БизнесПроцессы.ТипВсеСсылкиТочекМаршрутаБизнесПроцессов().СодержитТип(ТипРеквизита) Тогда
				...
				резДополнительныйТипЗначения = ТипЗначенияТочкиБизнесПроцессаСтрокой(ЗначениеРеквизита);
			Иначе
				СпецРежимПоиска = "";
				Если СвязьПоГуид Тогда
...
Процедура ЗаписьДанныхОбъектногоТипа(СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, массивДанных)
	...
	ЗапросИЗапись(РезультатЗапроса, ОбъектыВыгруженныеСОшибками, массивДанных, Истина);
...
//
Процедура ЗапросИЗапись(РезультатЗапроса, ОбъектыВыгруженныеСОшибками, массивДанных, ЗапросВерхнегоУровня = Ложь)

	// универсальная процедура выгрузки ссылочных объектов процедура
	ОбработкаРезультатаЗапроса(РезультатЗапроса, ОбъектыВыгруженныеСОшибками,
		массивДанных, МассивИмен, ЗапросВерхнегоУровня, Истина);
...
//
Процедура ОбработкаРезультатаЗапроса(РезультатЗапроса, ОбъектыВыгруженныеСОшибками,
		массивДанных, МассивИмен, ЗапросВерхнегоУровня = Ложь, ЭтоЗапросПоОбъекту = Ложь)
...
//
Процедура ЗаписьРегистра(СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, РегистрБухгалтерии = Ложь, массивДанных = Неопределено)
	...
	ЗаписьЧерезНаборЗаписей(МенеджерНабораЗаписей, ИмяТаблицыДляЗапроса,
		...
		РегистрБухгалтерии, массивДанных);
...
Процедура ЗаписьЧерезНаборЗаписей(МенеджерНабораЗаписей, ДляЗапроса, ИмяОбъекта,
	СтрокаДереваМетаданных, ОбъектыВыгруженныеСОшибками, РегистрБухгалтерии, массивДанных)
...
Функция ПолучитьДоступныеОбщиеРеквизитыОбъекта(Объект)
